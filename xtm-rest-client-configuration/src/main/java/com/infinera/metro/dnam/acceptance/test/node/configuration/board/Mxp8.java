...
public class Mxp8 extends AbstractClientAndLinePortBoard implements MUX_IF_MUX_LinePortBoard, MRT_IF_MRT_ClientPortBoard {

    @Builder
    @java.beans.ConstructorProperties({"subrack", "slot", "boardAttributes", "clientPorts", "linePorts"})
    private Mxp8(Subrack subrack, Slot slot, @Singular List<BoardAttributes> boardAttributes, @Singular  List<ClientPort> clientPorts, @Singular List<LinePort> linePorts) {
        super(BoardType.MXP8, subrack, slot, boardAttributes, clientPorts, linePorts);
    }
}
